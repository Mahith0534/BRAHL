...
3. Start: `python BRAHL_Local/run_local.py` → http://127.0.0.1:8766
...
Hard stops apply: monthly user tokens, project tokens, and automation wallet pool. Usage is stored in `BRAHL_Local/data/ai_usage.json` and mirrored on `project.ai_usage`.

## GitHub desktop packaging notes

- Ship `FoXYiZ/` (`f`/`x`/`y`) + `BRAHL_Local/` + lean samples (e.g. Math).
