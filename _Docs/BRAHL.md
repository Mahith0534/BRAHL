...
| **Do not use for** | Editing frozen engine binaries; product UI code → `BRAHL/` |
| **Related skills** | `foxyiz` (how to run/author) |
...
- **OpenAI** — set `OPENAI_API_KEY` (you pay usage), or  
- **Ollama** — free local model: `OPENAI_API_KEY=ollama`, `OPENAI_BASE_URL=http://127.0.0.1:11434/v1`, `OPENAI_MODEL=llama3.2`

Restart BRAHL after changing `.env`. Details: [FoXYiZ.md](FoXYiZ.md) (`f/` layout).
