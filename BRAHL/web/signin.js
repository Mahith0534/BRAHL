...
function selectProfile(id) {
  ...
  if (!profile) return;
  saveProfile(id);
  localStorage.setItem("brahl_local_avatar", profile.defaultAvatar);
  const params = new URLSearchParams(location.search);
  const suite = params.get("suite");
  if (profile.landing === "signin" || profile.role === "new-user") {
    localStorage.removeItem("brahl_local_suite");
    localStorage.removeItem("brahl_local_project_id");
  } else if (suite) {
    localStorage.setItem("brahl_local_suite", suite);
  }
...
function bindReset() {
  $("#signin-reset")?.addEventListener("click", () => {
    ...
    localStorage.removeItem("brahl_local_avatar");
    localStorage.removeItem("brahl_local_project_id");
    localStorage.removeItem("brahl_local_suite");
    renderProfiles();
  });
