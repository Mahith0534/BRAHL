...
const STORAGE_AVATAR = "brahl_local_avatar";
const STORAGE_PROJECT = "brahl_local_project_id";
const STORAGE_SUITE = "brahl_local_suite";
...
if (DESKTOP_MODE) document.body.classList.add("desktop-mode");
...
function syncAvatarSections() {
  ...
  $$(".avatar-client-only").forEach((el) => {
    // Desktop: never resurface wallet / hunter-invite / Advanced / AI chat chrome
    if (
      DESKTOP_MODE &&
      (el.classList.contains("desktop-hide-wallet") ||
        el.classList.contains("desktop-hide-build-extra"))
    ) {
      el.hidden = true;
...
async function loadPersonaTasks() {
  ...
  if (sessionStorage.getItem("brahl_local_persona_badge_dismissed") === "1") {
    strip.hidden = true;
  ...
  try {
    ...
    if (fixture?.fixture) {
      sessionStorage.setItem("brahl_local_persona_fixture", JSON.stringify(fixture.fixture));
    }
  } catch {
    strip.hidden = true;
  }
...
function bindPersonaBadge() {
  ...
  dismissBtn?.addEventListener("click", () => {
    sessionStorage.setItem("brahl_local_persona_badge_dismissed", "1");
    if (strip) strip.hidden = true;
    if (reopen) reopen.hidden = false;
  });
  ...
  reopen?.addEventListener("click", () => {
    sessionStorage.removeItem("brahl_local_persona_badge_dismissed");
    if (reopen) reopen.hidden = true;
    loadPersonaTasks();
  });
}
...
function inferSuiteFromProject(project) {
  ...
  return state.suites.find((s) => s.name === "BRAHL_Local")?.name || state.suites[0]?.name || null;
}
...
async function populateRunSelect(sel) {
  const suite = state.suiteName || "BRAHL_Local";
  ...
  const rows = await Promise.all(
    runNames.map(async (name) => {
      ...
      try {
        ...
        shotCount = (arts.counts?.screenshots || 0) + (arts.counts?.overlays || 0);
      } catch {
        /* optional */
      }
      const link = href
      ...
      const film =
        shotCount > 0
          ? `<button type="button" class="secondary link-btn sm js-batch-film" data-run="${escapeHtml(name)}">Film roll</button>`
          : "";
      const useBtn = `<button type="button" class="linkish js-batch-select" data-run="${escapeHtml(name)}" title="Use for Open zDash / film roll">Use</button>`;
      return `<div class="run-batch-row"><span class="run-batch-name">${escapeHtml(name)}</span>${useBtn}${link}${film}</div>`;
    })
  );
  let batchLinkHtml = "";
  if (batchDashboard) {
    ...
    batchLinkHtml =
      `<p class="run-batch-primary"><a href="/api/files/z-root/${encodeURIComponent(name)}" target="_blank" class="secondary link-btn">Combined batch zDash</a> <span class="hint">primary for parallel fan-out</span></p>`;
  }
  wrap.innerHTML =
    `<h4 class="section-sub">Batch jobs (${runNames.length})</h4>` + batchLinkHtml + rows.join("");
  wrap.hidden = false;
  wrap.querySelectorAll(".js-batch-film").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const run = btn.getAttribute("data-run");
      await makeFilmRollForRun(run, batchDashboard);
    });
  });
  wrap.querySelectorAll(".js-batch-select").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const run = btn.getAttribute("data-run");
      selectedRun = run;
      await showRunPostActions(run, batchDashboard);
      setStatus(`Selected job ${run} for zDash / film roll`);
    });
  });
}

async function makeFilmRollForRun(runName, batchDashboard) {
  if (!runName) return;
  setStatus(`Building film roll for ${runName}…`);
  try {
    await api(`/api/runs/${encodeURIComponent(runName)}/filmstrip`, { method: "POST", body: "{}" });
    setStatus(`Film roll ready for ${runName}`);
...
function applyCaptureFormToJson() {
  ...
  refreshHeavyCaptureWarn(parsed.capture);
}
...
function isHeavyCapture(capture) {
  const c = { ...CAPTURE_DEFAULTS, ...(capture || {}) };
  const imageHeavy = ["every_step", "on_pass"].includes(String(c.image || ""));
  const videoHeavy = ["plan", "every_step"].includes(String(c.video || ""));
  return imageHeavy || videoHeavy;
}

function suiteAutomatedCount() {
  const auto = ypadState.insights?.automated;
  if (typeof auto === "number") return auto;
  const suite = state.suites.find((s) => s.name === state.suiteName);
  const n = suite?.plan_run_y;
  return typeof n === "number" ? n : null;
}

/** Pass line that clarifies tag-filtered runs vs full suite automation. */
function formatRunPassSummary(r, { compact = false } = {}) {
  const passes = r.passes ?? 0;
  const total = r.total_plans ?? passes + (r.fails ?? 0);
  const fails = r.fails ?? 0;
  const tags = Array.isArray(r.tags) ? r.tags.filter(Boolean) : [];
  const tagBit = tags.length ? ` · tags ${tags.join("+")}` : " · tag-filtered run";
  const suiteAuto = suiteAutomatedCount();
  let scope = "";
  if (suiteAuto != null && total < suiteAuto) {
    scope = compact
      ? ` · ${total} of ${suiteAuto} suite auto`
      : ` · ${total} of ${suiteAuto} Run=Y plans in filter`;
  } else if (!tags.length && suiteAuto != null && total >= suiteAuto) {
    scope = compact ? "" : " · full filter";
  }
  return `${passes}/${total} pass · ${fails} fail${tagBit}${scope}`;
}

function refreshHeavyCaptureWarn(capture) {
  // Run page no longer shows capture banners; keep modal warn only.
  const modalWarn = $("#fstart-capture-heavy-warn");
  if (!modalWarn) return;
  const threads = runThreadCount();
  const heavy = isHeavyCapture(capture);
  const fanout = threads > 1 && selectedRunTags().length >= 2;
  const msg =
    heavy && (fanout || threads > 1)
      ? `Heavy capture × up to ${Math.min(threads, Math.max(1, selectedRunTags().length))} jobs — expect large z/. Soften image/video or re-run pyUtils/cleaner.py after.`
      : heavy
        ? "Heavy capture (every_step / video=plan) — z/ can grow fast on long suites."
        : "";
  if (msg) {
    modalWarn.textContent = msg;
...
async function refreshRunCaptureStrip() {
  // Capture strip removed from Run — settings live in fStart Edit only.
  const path = $("#config-select")?.value;
  if (!path) {
    refreshHeavyCaptureWarn(null);
    return;
  }
  try {
    const data = await api(`/api/configs/content?path=${encodeURIComponent(path)}`);
    refreshHeavyCaptureWarn(data.content?.capture);
  } catch {
    refreshHeavyCaptureWarn(null);
  }
}
...
function renderArtifactActionsHtml(arts, { asButtons = true, linkClass = "linkish" } = {}) {
  if (!arts) return "";
  const bits = [];
  if (arts.visual_playback?.url) {
    bits.push(
      `<a href="${escapeHtml(arts.visual_playback.url)}" target="_blank" class="${linkClass}">Visual playback</a>`
    );
  }
  const shotCount = (arts.counts?.screenshots || 0) + (arts.counts?.overlays || 0);
  if (shotCount > 0 && asButtons) {
    bits.push(
      `<button type="button" class="${linkClass} js-open-shots" data-run="${escapeHtml(arts.run_name)}">Screenshots (${shotCount})</button>`
    );
  }
  if (arts.gif?.url) {
    bits.push(`<a href="${escapeHtml(arts.gif.url)}" target="_blank" class="${linkClass}">GIF</a>`);
  }
...
async function showRunPostActions(runName, batchDashboard) {
  ...
  if (makeRoll) {
    makeRoll.hidden = true;
    makeRoll.onclick = null;
  }
  ...
  if (hasRun) {
    try {
      ...
      if (makeRoll && shotCount > 0) {
        ...
        makeRoll.textContent = arts.gif ? "Remake film roll" : "Film roll";
        makeRoll.onclick = async () => {
          makeRoll.disabled = true;
          try {
            await makeFilmRollForRun(runName, batchDashboard);
          } finally {
        ...
        rollHint.textContent = hasBatch
          ? "No PNGs in this job — pick another batch job (Use / Film roll) or set Capture image=on_fail with failures / every_step."
          : "No PNGs in this run — set Capture image=on_fail (with failures) or every_step to enable film roll.";
      }
    } catch {
      /* artifacts optional */
    }
    ...
    list.innerHTML = rows
      .map((r) => {
        ...
        return (
          ...
          `<span class="meta">${when ? escapeHtml(when) + " · " : ""}${escapeHtml(formatRunPassSummary(r, { compact: true }))}</span>` +
          `${dashBtn}` +
...
let runTagCounts = {};
/** Collapsed Run tags row — show a few key chips; expand for the rest. */
let runTagsExpanded = false;
const RUN_TAG_COLLAPSED_MAX = 5;
const RUN_TAG_PRIORITY = ["Smoke", "UI", "API", "Auth", "Security", "Performance", "Manual"];
...
/** Parallel execution is derived only from Parallel > 1 + 2+ tags — no separate control. */
...
async function refreshFstartFanoutHint() {
  ...
  const overrideNote = "Run tags override fStart for this job.";
  if (threads > 1 && tags.length >= 2) {
    const workers = Math.min(threads, tags.length);
    hint.textContent = `${overrideNote} Parallel ${threads} · ${tags.length} tags → ${workers} workers.`;
    hint.hidden = false;
  } else {
    hint.textContent = `${overrideNote} tags: ${tags.join(", ")}.`;
    hint.hidden = false;
  }
  try {
    const path = $("#config-select")?.value;
    if (path) {
      ...
      refreshHeavyCaptureWarn(readCaptureForm());
    }
  } catch {
    refreshHeavyCaptureWarn(readCaptureForm());
  }
...
function runTagVisibleOrder() {
  if (runTagsExpanded || runTagOrder.length <= RUN_TAG_COLLAPSED_MAX) {
    return [...runTagOrder];
  }
  const byCount = [...runTagOrder].sort((a, b) => {
    const ca = Number(runTagCounts[a] || 0);
    const cb = Number(runTagCounts[b] || 0);
  ...
  const seen = new Set();
  const take = (t) => {
    if (!t || seen.has(t) || !runTagOrder.includes(t)) return;
    seen.add(t);
    picked.push(t);
  };
  // Prefer familiar demo/capability tags first
  for (const p of RUN_TAG_PRIORITY) {
    const hit = runTagOrder.find((t) => t.toLowerCase() === p.toLowerCase());
    take(hit);
    if (picked.length >= RUN_TAG_COLLAPSED_MAX) break;
  }
  for (const t of byCount) {
    if (picked.length >= RUN_TAG_COLLAPSED_MAX) break;
    take(t);
  }
  // Always keep active selections visible while collapsed
  for (const t of runTagOrder) {
    if (runTagsSelected.has(t)) take(t);
  }
  return picked;
}
...
function renderRunTagChips() {
  ...
  if (!row) return;
  if (!runTagOrder.length) {
    row.innerHTML = `<span class="hint">No suite tags in yPAD — quick picks below (or add Tags on plans).</span>`;
    return;
  }
  ...
  const visible = runTagVisibleOrder();
  const hiddenCount = Math.max(0, runTagOrder.length - visible.length);
  const chips = visible
    .map((t) => {
    ...
    .join("");
  let toggle = "";
  if (runTagOrder.length > RUN_TAG_COLLAPSED_MAX) {
    if (runTagsExpanded) {
      toggle = `<button type="button" class="fstart-chip run-tag-toggle" id="run-tag-toggle" aria-expanded="true">Show less</button>`;
    } else {
      toggle = `<button type="button" class="fstart-chip run-tag-toggle" id="run-tag-toggle" aria-expanded="false">+${hiddenCount} more</button>`;
    }
  }
  row.innerHTML = chips + toggle;
  row.querySelectorAll(".run-tag-chip").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      // Re-render so newly selected tags stay visible when collapsed
      renderRunTagChips();
      syncPresetChipHighlight();
      syncConfigSelectFromChips();
    });
  });
  $("#run-tag-toggle")?.addEventListener("click", () => {
    runTagsExpanded = !runTagsExpanded;
    renderRunTagChips();
  });
}
...
function renderRunProfileChips() {
  const row = $("#run-profile-row");
  const presetsWrap = $("#run-presets-wrap");
  if (!row) return;
  // Suite already exposes Run tags — hide redundant Presets/quick-picks to avoid dual UI.
  if (presetsWrap) {
    const showFallback = !runTagOrder.length && runProfileOrder.length > 0;
    presetsWrap.hidden = !showFallback;
  }
  ...
  row.innerHTML = runProfileOrder
    .map((p) => {
      ...
      return `<button type="button" class="fstart-chip run-profile-chip${runProfilesSelected.has(p) ? " selected" : ""}" data-profile="${escapeHtml(p)}" title="Select matching suite tags">${escapeHtml(label)}</button>`;
    })
    .join("");
...
async function loadAppVersion() {
  try {
    ...
    if (foot) foot.textContent = `BRAHL Local v${d.version}`;
  } catch {
    /* keep static footer */
  }
}
...
function updateHealHint() {
  ...
  if (!runName) {
    hint.textContent = "Select a run in Analyze (or complete a Run first).";
    return;
  }
  const src = selectedRun ? "selected" : "latest";
  hint.textContent = `Using ${src} run: ${runName}`;
}
...
async function refreshArenaCostWidget() {
  ...
  if (DESKTOP_MODE) {
    ...
    else if (dock && state.suiteName) dock.hidden = false;
    refreshArenaYpadWidget();
...
async function acceptBrahlPlan() {
  ...
  try {
    ...
    const ypad = data.ypad || data.project?.ypad || {};
    const added = ypad.added_plans ?? ypad.automated_plans;
    if (ypad.mode === "merge" && (added === 0 || ypad.message)) {
      setStatus(
        ypad.message ||
          "Plan accepted — no new titles to add (already in yPAD). Generate again with different cases, or edit titles."
      );
    } else if (ypad.mode === "merge") {
      setStatus(
        `Added ${ypad.added_plans || 0} test(s) to yPAD` +
          (ypad.skipped_existing ? ` · skipped ${ypad.skipped_existing} duplicate title(s)` : "")
      );
    } else {
...
const STORAGE_XP = "brahl_local_xp";
const STORAGE_A77_LOCAL = "brahl_local_atomic77_local";
const XP_WEIGHTS = {
...
function sessionMinutesEstimate() {
  const key = "brahl_local_session_start";
        ...
        .map((v) => {
          ...
          return `<li class="ypad-version-item${active}" data-vid="${escapeHtml(v.id)}">
            ...
            <div class="ypad-version-actions">
              ...
              <button type="button" class="linkish" data-act="merge">Add missing plans</button>
  ...
  try {
    if (act === "diff") {
      ...
      if (el) {
        el.hidden = false;
        el.innerHTML =
          `<strong>Diff vs current</strong> — +${diff.counts?.added || 0} added in live / −${diff.counts?.removed || 0} only in snapshot / ~${diff.counts?.changed || 0} same id, different fields` +
          `<p class="meta" style="margin:0.4rem 0 0">Add missing plans copies snapshot-only rows into live yPAD (never deletes). Same-id rows (~) are skipped — use <strong>Accept &amp; add tests</strong> on Rebuild to append newly generated titles.</p>` +
    ...
    if (act === "merge") {
      ...
      const n = res.merged || 0;
      if (n === 0) {
        setStatus(res.message || "Nothing to add — snapshot plans already in the live yPAD");
      } else {
        setStatus(res.message || `Added ${n} plan(s) from snapshot ${versionId}`);
      }
      await loadYpadSheet(ypadState.tab);
      await loadBuildBoard();
      return;
...
function renderBuildBoard(data) {
  ...
  if (verifyEl) {
    // Desktop Build header stays clean — no FoXYiZ / latest-verify under the title
    if (DESKTOP_MODE) {
      verifyEl.hidden = true;
      verifyEl.setAttribute("aria-hidden", "true");
    } else {
      const st = data.report_stats;
      if (st && data.latest_run) {
        verifyEl.hidden = false;
        verifyEl.textContent =
          `Latest verify: ${formatRunPassSummary({ ...st, tags: st.tags || [] })} · run ${data.latest_run} · full BRAHL report on the BRAHL tab`;
      } else {
        verifyEl.hidden = true;
      }
    }
...
function refreshLoopBuiltSummary() {
  const statusEl = $("#loop-status-line");
  const suite = state.suiteName || activeProject?.suite_name || "—";
  const latestRaw = activeProject?.latest_run || "";
  const latest =
    suite && latestRaw.endsWith(`_${suite}`) ? latestRaw : "";
  if (statusEl) {
    statusEl.textContent = latest
      ? `Suite ${suite} · latest run ${latest}`
      : `Suite ${suite} · no run yet`;
  }
  const purpose =
...
async function loadRuns() {
  ...
  if (!list) return;
  if (!projectSuite) {
    list.innerHTML = `<li class="empty-hint">Select a project to see its z/ runs.</li>`;
    return;
  }
  const { runs } = await api(`/api/runs?suite=${encodeURIComponent(projectSuite)}`);
  if (!runs.length) {
    list.innerHTML = `<li class="empty-hint">No z/ runs for ${escapeHtml(projectSuite)} yet.</li>`;
    return;
  }
  list.innerHTML = "";
  let firstLi = null;
  ...
  const preferredName = activeProject?.latest_run || "";
  const preferred =
    (preferredName && runs.find((r) => r.name === preferredName)) || firstRun;
  if (preferred) {
    const preferLi =
      [...list.querySelectorAll(":scope > li")].find((el) =>
        el.querySelector(".run-name")?.textContent === preferred.name
      ) || firstLi;
...
async function startRun(stepLabel) {
  ...
  const tags = selectedRunTags();
  if (!tags.length) {
    alert("Select at least one Run tag.");
    return null;
  }
  const threads = runThreadCount();
  ...
  if (stepLabel === "Run") {
    setStatus(`Running with tags ${tags.join("+")} (overrides fStart tags) · parallel ${threads}`);
  }
          ...
          if (j.status === "completed" && runName) {
            ...
            if (stepLabel === "Run") {
              if (isBatch) {
                ...
                setStatus(
                  `Batch complete — ${allRunDirs.length} job(s). Combined batch zDash is primary; Use a job for film roll.`
                );
              } else {
                setStatus(`Run complete (${tags.join("+")}) — review in Analyze or open zDash`);
              }
            }
          }
          if (j.status === "completed" && runName) {
...
async function openFstartEditor() {
  ...
  try {
    ...
    $("#fstart-modal-title").textContent = "Edit fStart";
    const pathEl = $("#fstart-modal-path");
    if (pathEl) {
      const full = data.path || "";
      pathEl.textContent = full.split(/[/\\]/).pop() || full;
      pathEl.title = full;
      pathEl.dataset.fullPath = full;
    }
    ...
    const details = $("#fstart-modal .fstart-json-details");
...
async function saveFstartEditor() {
  ...
  parsed.capture = readCaptureForm();
  const path = fstartEditorPath || $("#fstart-modal-path")?.dataset?.fullPath || $("#fstart-modal-path")?.title?.trim();
  if (!path) return;
  try {
...
async function saveRunTagsToFstart() {
  const path = fstartEditorPath || $("#config-select")?.value;
  if (!path) {
    setStatus("No fStart selected");
    return;
  }
  const tags = selectedRunTags();
  if (!tags.length) {
    setStatus("Select at least one Run tag first");
    return;
  }
  try {
    const data = await api(`/api/configs/content?path=${encodeURIComponent(path)}`);
    const content = { ...(data.content || {}), tags };
    await api("/api/configs/content", {
      method: "PUT",
      body: JSON.stringify({ path, content }),
    });
    if (!$("#fstart-modal")?.hidden) {
      $("#fstart-json-editor").value = JSON.stringify(content, null, 2);
      fillCaptureForm(content.capture);
    }
    await loadConfigsForSuite();
    const sel = $("#config-select");
    if (sel) sel.value = path.replace(/\\/g, "/");
    setStatus(`Saved Run tags [${tags.join(", ")}] → ${path}`);
  } catch (e) {
    setStatus(`Save Run tags failed: ${e.message}`);
  }
}
...
async function refreshWorkspaceChrome() {
  ...
  try {
    ...
    if (label) {
      if (data.bound) {
        const short = data.short_label || data.workspace?.slug || "bound";
        const path = data.workspace?.local_path || "";
        const looksLikeTool =
          /[/\\]KK2\b/i.test(path) || /[/\\]FoXYiZ\b/i.test(path) || /[/\\]BRAHL_Local\b/i.test(path);
        label.textContent = looksLikeTool ? `${short} (tool folder?)` : short;
        label.title = looksLikeTool
          ? `${path}\nTip: bind the app under test, not the BRAHL/KK2 tool repo.`
          : path;
      } else {
        label.textContent = "not set";
        label.title = "Bind the app under test (not this BRAHL tool folder)";
      }
    }
...
function showWorkspaceGate(show) {
  ...
  const closeBtn = $("#btn-workspace-gate-close");
  // Allow close when a workspace is already bound; first-run must pick one.
  if (closeBtn) closeBtn.hidden = !desktopWorkspace;
  if (show && desktopWorkspace) {
    const ws = desktopWorkspace;
    const urlEl = $("#workspace-repo-url");
    const branchEl = $("#workspace-branch");
    const localEl = $("#workspace-local-path");
    if (urlEl) urlEl.value = ws.repo_url || ws.git_url || "";
    if (branchEl) branchEl.value = ws.default_branch || ws.branch || "";
    if (localEl) localEl.value = ws.local_path || "";
  }
}

function openWorkspaceGate() {
  showWorkspaceGate(true);
}

function closeWorkspaceGateIfAllowed() {
  if (!desktopWorkspace) {
    setStatus("Bind an app under test first (GitHub or local folder).");
    return;
  }
...
function initWorkspaceGate() {
  ...
  $("#btn-workspace-gate-close")?.addEventListener("click", closeWorkspaceGateIfAllowed);
  const wrap = $("#topbar-workspace");
  wrap?.addEventListener("click", openWorkspaceGate);
  ...
  document.addEventListener("keydown", (e) => {
    ...
    const gate = $("#workspace-gate");
    if (gate && !gate.hidden) closeWorkspaceGateIfAllowed();
  });
  // Click backdrop to close when already bound
  $("#workspace-gate")?.addEventListener("click", (e) => {
    if (e.target === e.currentTarget) closeWorkspaceGateIfAllowed();
  });
}
...
document.querySelector("#phase-nav")?.addEventListener(
  "click",
  async (e) => {
    const link = e.target.closest(".phase-path");
    if (!link || !document.querySelector("#phase-nav")?.contains(link)) return;
    e.preventDefault();
    e.stopPropagation();
    const which = (link.dataset.reveal || "").toLowerCase();
    if (which !== "y" && which !== "z") return;
    const suite =
      which === "y"
        ? inferSuiteFromProject(activeProject) || activeProject?.suite_name || null
        : null;
    try {
      const data = await api("/api/reveal-folder", {
        method: "POST",
        body: JSON.stringify({ which, suite }),
      });
      const path = data.path || `/${which}`;
      setStatus(`Opened ${path}`);
      // Visible confirmation — Explorer can open behind the browser
      showRevealToast(path, data.root || "");
    } catch (err) {
      console.error("reveal-folder failed", err);
      setStatus(err.message || `Could not open /${which}`);
      alert(
        `Could not open /${which} on desktop.\n\n${err.message || err}\n\n` +
          "Restart BRAHL Local (Run BRAHL Local.bat) and hard-refresh this page."
      );
    }
  },
  true
...
function showRevealToast(path, root) {
  let toast = $("#reveal-folder-toast");
  if (!toast) {
    toast = document.createElement("div");
    toast.id = "reveal-folder-toast";
    toast.className = "reveal-folder-toast";
    toast.setAttribute("role", "status");
    document.body.appendChild(toast);
  }
  toast.innerHTML =
    `<strong>Opened folder</strong>` +
    `<code class="reveal-folder-path">${escapeHtml(path)}</code>` +
    (root ? `<span class="reveal-folder-root">FoXYiZ root: ${escapeHtml(root)}</span>` : "") +
    `<span class="reveal-folder-hint">If you don’t see Explorer, check the taskbar — it may be behind this window.</span>` +
    `<button type="button" class="secondary reveal-folder-copy">Copy path</button>` +
    `<button type="button" class="secondary reveal-folder-dismiss">OK</button>`;
  toast.hidden = false;
  toast.querySelector(".reveal-folder-copy")?.addEventListener("click", async () => {
    try {
  ...
  toast.querySelector(".reveal-folder-dismiss")?.addEventListener("click", () => {
    toast.hidden = true;
  });
  clearTimeout(showRevealToast._t);
  showRevealToast._t = setTimeout(() => {
    toast.hidden = true;
  }, 12000);
}
...
$("#fstart-save-run-tags")?.addEventListener("click", saveRunTagsToFstart);
...
$("#btn-build-edit-project")?.addEventListener("click", () => {
  const plan = $("#build-brahl-plan");
  const btn = $("#btn-build-edit-project");
  if (!plan) return;
  const opening = plan.hidden || !plan.open;
  if (opening) {
    plan.hidden = false;
    plan.open = true;
    if (btn) {
      btn.textContent = "Done";
      btn.title = "Collapse rebuild plan";
    }
    plan.scrollIntoView({ behavior: "smooth", block: "nearest" });
    $("#brahl-plan-requirement")?.focus();
  } else {
    plan.open = false;
    if (btn) {
      btn.textContent = "Rebuild";
      btn.title = "Rebuild plan / expand coverage";
    }
  }
});
$("#build-brahl-plan")?.addEventListener("toggle", () => {
  const plan = $("#build-brahl-plan");
  const btn = $("#btn-build-edit-project");
  if (!plan || !btn) return;
  if (plan.hidden || !plan.open) {
    btn.textContent = "Rebuild";
    btn.title = "Rebuild plan / expand coverage";
  } else {
    btn.textContent = "Done";
...
$("#tagline-foxyiz")?.addEventListener("click", () => {
  if (typeof openAiDocsModal === "function") {
    openAiDocsModal();
    setStatus("FoXYiZ docs — general layer + project .md");
  }
});
$("#btn-rebuild-attach")?.addEventListener("click", () => {
  const ta = $("#brahl-plan-requirement");
  const url = ($("#rebuild-thought-url")?.value || "").trim();
  const file = $("#rebuild-thought-pdf")?.files?.[0];
  const status = $("#rebuild-attach-status");
  const bits = [];
  if (url) bits.push(`Thought URL: ${url}`);
  if (file) bits.push(`PDF attached (local): ${file.name}`);
  if (!bits.length) {
    if (status) {
      status.hidden = false;
      status.textContent = "Add a URL and/or choose a PDF first.";
    }
    return;
  }
  ...
  if (status) {
    status.hidden = false;
    status.textContent = "Attached to rebuild notes below. Capture more on ThoughtStream if needed.";
  }
  setStatus("Rebuild notes updated");
});
...
window.addEventListener("qoa-theme-change", () => {
  ...
  if (title) title.textContent = "BRAHL Local";
});
applyAvatarLabelsToDom();
applyAvatarModeNav();
loadAppVersion();
