"""BRAHL_Local — local BRAHL web API + static frontend."""
...
# BRAHL_Local — no marketplace/cloud surface
DESKTOP_MODE = (
    os.environ.get("BRAHL_LOCAL")
    or os.environ.get("QOA_DESKTOP")
    or "1"
...
def _startup() -> None:
    # Load BRAHL_Local/.env into process env if present (production host convenience).
...
class ThoughtCaptureCreate(BaseModel):
    title: str
    details: str = ""
    rating: str = "issue"  # issue | critical | feature
    run_name: str | None = None
    evidence_ids: list[str] | None = None
    author: str = ""
...
def version() -> dict[str, str]:
    return {"version": APP_VERSION, "service": "BRAHL_Local", "brahl_phases": "Build,Run,Analyze,Heal,Loop,BRAHL"}
...
def reveal_folder(body: RevealFolderRequest) -> dict[str, Any]:
    """Open FoXYiZ y/ or z/ (or suite subfolder) in Windows Explorer / Finder."""
    import os
    import platform
    import subprocess

    from paths import FOXYIZ_ROOT, Y_DIR, Z_DIR

    which = (body.which or "").strip().lower()
    if which not in ("y", "z"):
        raise HTTPException(400, "which must be 'y' or 'z'")
    base = Y_DIR if which == "y" else Z_DIR
    target = base
    suite = (body.suite or "").strip()
    if suite and which == "y":
        safe = Path(suite).name
        cand = (Y_DIR / safe).resolve()
        y_root = Y_DIR.resolve()
        if cand.is_dir() and (cand == y_root or y_root in cand.parents):
            target = cand
    ...
    try:
        ...
        if system == "Windows":
            # Multiple strategies — Explorer often ignores a bare path when already open.
            launched = False
            for cmd in (
                ["explorer", path],
                ["explorer", "/e,", path],
            ...
            if not launched:
                raise OSError("; ".join(errors) or "no launcher worked")
        elif system == "Darwin":
...
def brahl_plan_accept(project_id: str, body: BrahlPlanAcceptRequest, request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(404, "Project not found") from None
    ypad = project.get("ypad") if isinstance(project, dict) else None
    return {"project": project, "ypad": ypad}
...
def ai_status(request: Request, project_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "setup_hint": (
            "Desktop BYOK: set OPENAI_API_KEY in FoXYiZ/f/.env (or Ollama via OPENAI_BASE_URL). "
...
def heal_suggest_ai(project_id: str, run_name: str, body: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    suite_config = project.get("suite_config") or f"y/{project.get('suite_name', 'BRAHL_Local')}/{project.get('suite_name', 'BRAHL_Local')}.json"
    if not ai_assist.is_ai_available():
...
def heal_apply_patches(project_id: str, body: dict[str, Any]) -> dict[str, Any]:
    ...
    suite_config = (
        ...
        or f"y/{project.get('suite_name', 'BRAHL_Local')}/{project.get('suite_name', 'BRAHL_Local')}.json"
    )
...
def build_board(project_id: str, request: Request) -> dict[str, Any]:
    project = _require_project(project_id, request)
    suite_name = project.get("suite_name") or "BRAHL_Local"
...
@app.get("/api/projects/{project_id}/thought-captures")
def list_thought_captures(project_id: str, rating: str | None = None) -> dict[str, Any]:
    try:
        items = project_store.list_thought_captures(project_id, rating=rating)
        stats = project_store.thought_capture_stats(project_id)
    except KeyError:
        raise HTTPException(404, "Project not found") from None
    return {"items": items, "count": len(items), "stats": stats}


@app.post("/api/projects/{project_id}/thought-captures")
def create_thought_capture(project_id: str, body: ThoughtCaptureCreate) -> dict[str, Any]:
    if not (body.title or "").strip():
        raise HTTPException(400, "Title is required")
    try:
        item = project_store.add_thought_capture(
            project_id,
            title=body.title,
