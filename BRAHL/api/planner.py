...
# Bare domains like varkasa.com (optional scheme / www / path)
...
def _extract_url(text: str) -> str:
    ...
    if not m:
        return ""
    host = m.group(1).rstrip(").,;]").strip().lower()
...
def _is_weak_name(name: str) -> bool:
    ...
    return (not n) or n in _NAME_STOPWORDS or n in ("new_app", "project")
...
def _merge_draft(draft: dict[str, Any] | None, user_text: str) -> dict[str, Any]:
    ...
    if url:
        d["app_url"] = url
    # Prefer host-based name; upgrade filler names like "give" once URL is known
    if _is_weak_name(d.get("name") or ""):
...
def create_from_planner(
    ...
    raw_name = (draft.get("name") or "").strip()
    if _is_weak_name(raw_name):
        raw_name = _guess_name(app_url, purpose)
    name = slug_suite_name(raw_name)
    if not name:
