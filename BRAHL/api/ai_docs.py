...
Built-in docs live under Docs/ (architect) or _Docs/ (end-user package).
User docs live under BRAHL/data/user_ai_docs/ with a sidecar manifest.json.
"""
...
_BRAHL_ROOT = Path(__file__).resolve().parents[1]
KK_ROOT = _BRAHL_ROOT.parent
USER_DOCS_DIR = _BRAHL_ROOT / "data" / "user_ai_docs"
...
# Paths use Docs/…; _resolve() also checks _Docs/… for the FoXYiZ_User package layout.
...
AI_CONTEXT_DOCS: list[dict[str, Any]] = [
    ...
    },
    {
        "id": "vision",
        "title": "Vision",
        "subtitle": "QA on Air — HITL + AI marketplace story",
        "path": "Docs/Vision.md",
        "kind": "skill",
        "in_prompt": False,
    },
    ...
    },
    {
        "id": "byok",
        "title": "Desktop BYOK",
        "subtitle": "User-owned OpenAI key · local Ollama · hosted quota",
    ...
    },
    {
        "id": "rules",
        "title": "Agent rules",
        "subtitle": "Team conventions · Heal scope",
        "path": "Docs/rules.md",
        "kind": "rules",
        "in_prompt": False,
    },
    {
        ...
        "subtitle": "Local Arena app · API · verify",
        "path": "BRAHL/README.md",
        "kind": "readme",
        "in_prompt": False,
    },
    {
        ...
        "subtitle": "Exe folder notes · fStart",
        "path": "FoXYiZ/f/README.txt",
        "kind": "readme",
        "in_prompt": False,
    },
    {
        ...
        "subtitle": "What's in this FoXYiZ_User tree",
        "path": "Summary.md",
        "kind": "readme",
        "in_prompt": False,
    },
...
def _resolve(rel: str) -> Path:
    """Resolve a catalog path under the package root.

    Architect trees use Docs/; the FoXYiZ_User zip ships the same content as _Docs/.
    Also checks FOXYIZ_ROOT for nested engine readmes.
    """
    rel_norm = (rel or "").replace("\\", "/").lstrip("/")
    if not rel_norm:
        return KK_ROOT
    primary = KK_ROOT / Path(rel_norm)
    candidates: list[Path] = [primary]
    if rel_norm.startswith("Docs/"):
        rest = rel_norm[len("Docs/") :]
        candidates.append(KK_ROOT / "_Docs" / Path(rest))
    elif rel_norm.startswith("_Docs/"):
        rest = rel_norm[len("_Docs/") :]
        candidates.append(KK_ROOT / "Docs" / Path(rest))
    foxy = os.environ.get("FOXYIZ_ROOT")
    if foxy:
        fr = Path(foxy)
        candidates.append(fr / Path(rel_norm))
        if rel_norm.startswith("FoXYiZ/"):
            candidates.append(fr / Path(rel_norm[len("FoXYiZ/") :]))
...
def _display_path(rel: str, resolved: Path) -> str:
    """Prefer a path relative to package root when the file was found via alias."""
    try:
        return resolved.resolve().relative_to(KK_ROOT.resolve()).as_posix()
    except ValueError:
        return rel.replace("\\", "/")
        ...
        out.append(
            {
                ...
                "path": _display_path(spec["path"], path) if exists else spec["path"],
...
def list_user_docs() -> list[dict[str, Any]]:
    ...
    for entry in man.get("docs") or []:
        ...
        exists = path.is_file()
        size = path.stat().st_size if exists else 0
        rel = f"BRAHL/data/user_ai_docs/{doc_id}.md"
        out.append(
            {
                "id": doc_id,
...
def get_ai_doc(doc_id: str, project: dict[str, Any] | None = None) -> dict[str, Any] | None:
    ...
    if spec:
            ...
            return {
                **spec,
                "content": (
                    ...
                    f"Looked under `{KK_ROOT.as_posix()}` (also tries `_Docs/` when path is `Docs/`)."
                ),
                "exists": False,
                "source": "builtin",
                "editable": False,
        ...
        return {
            ...
            "title": spec["title"],
            "subtitle": spec.get("subtitle", ""),
            "path": _display_path(spec["path"], path),
            "kind": spec.get("kind", "doc"),
            "in_prompt": bool(spec.get("in_prompt")),
            "exists": True,
            "content": content,
    ...
    return {
        ...
        "title": entry.get("title") or doc_id,
        "subtitle": entry.get("subtitle") or "My doc",
        "path": f"BRAHL/data/user_ai_docs/{doc_id}.md",
        "kind": "user",
        "in_prompt": bool(entry.get("in_prompt")),
        "exists": exists,
        "content": content,
