"""FoXYiZ engine subprocess runner for BRAHL_Local API."""
...
def _backfill_job_log_from_zlogs(job: Job) -> None:
    """Replace pipe-captured console with zlogs.txt when available.

    FoXYiZ.exe often emits blank lines for emoji summary/status rows on Windows
    pipes (including Output Directory / Total Plans); on-disk zlogs.txt is complete.
    """
    _ensure_job_run_dirs(job)
    dirs: list[Path] = []
    seen: set[str] = set()
    for raw in [*(job.run_dirs or []), job.output_dir]:
        if not raw or "zDash_batch_" in str(raw):
            continue
        p = Path(str(raw))
    ...
    for d in dirs:
        ...
        if not zlog.is_file():
            continue
        try:
            body = zlog.read_text(encoding="utf-8", errors="replace").splitlines()
        except OSError:
            continue
        if not body:
            continue
        ...
        chunks.extend(body)
        # Prefer canonical output_dir from zlogs header when present
        for line in body[:12]:
            if line.startswith("output_dir="):
                out = line.split("=", 1)[1].strip().replace("\\", "/")
                if out:
                    job.output_dir = out
                break
    if not chunks:
        return
    # Keep BRAHL preamble ([i] Engine / Runtime fStart / cloud …)
    prefix = [ln for ln in job.log_lines if ln.startswith("[")]
...
def _ensure_job_run_dirs(job: Job) -> None:
    ...
    found: list[Path] = []
    for p in Z_DIR.iterdir():
        if not p.is_dir() or "zDash_batch_" in p.name:
            continue
    ...
    job.run_dirs = [str(p) for p in found]
    job.output_dir = str(found[-1])
...
def expand_run_profiles(profiles: list[str] | None) -> list[str]:
    ...
    for raw in profiles:
        ...
        # BRAHL_Local / A77 use Navigation; profile map historically said Nav
...
def _plan_tags_map(suite_name: str) -> dict[str, list[str]]:
    """PlanId → display tags from y/<suite>/y1Plans.csv (and sibling y1*.csv)."""
    suite_dir = Y_DIR / suite_name
    if not suite_dir.is_dir():
        return {}
    out: dict[str, list[str]] = {}
    suite_l = suite_name.lower()
...
def by_area_from_plan_results(
    suite_name: str,
    plan_results: dict[str, str],
) -> list[dict[str, Any]]:
    """Aggregate pass/fail by yPAD tag (UI, API, Security, Performance, …)."""
    if not plan_results:
        return []
    plan_tags = _plan_tags_map(suite_name)
    areas: dict[str, dict[str, Any]] = {}
    for plan_id, result in plan_results.items():
        tags = plan_tags.get(plan_id) or ["Untagged"]
        for tag in tags:
            bucket = areas.setdefault(
                tag.lower(),
                {"area": tag, "passes": 0, "fails": 0, "total": 0},
            )
            bucket["total"] += 1
            if result == "fail":
                bucket["fails"] += 1
    ...
    return sorted(
        areas.values(),
        key=lambda a: (-int(a["fails"]), -int(a["total"]), str(a["area"]).lower()),
    )


def _tags_from_zlogs(run_dir: Path) -> list[str]:
    """Best-effort parse of engine tag filter from zlogs.txt."""
    log = run_dir / "zlogs.txt"
    if not log.is_file():
        return []
    try:
        ...
        return []
    m = re.search(r"Tag filter:\s*Running plans with tags:\s*(.+)", text, re.IGNORECASE)
    if not m:
        if re.search(r"Tag filter:.*['\"]?All['\"]?", text, re.IGNORECASE):
            return ["All"]
        return []
    raw = m.group(1).strip()
    # Drop trailing progress / emoji noise on same line
    raw = re.split(r"[\r\n]|Progress:|Starting plan", raw, maxsplit=1)[0].strip()
    ...
    def worker() -> None:
        ...
        try:
            ...
            if ENGINE_EXE.is_file():
                job.log_lines.append(f"[i] Engine: FoXYiZ.exe ({ENGINE_EXE})")
            else:
                job.log_lines.append(f"[i] Engine: python {ENGINE}")
            _drain_stdout(proc, job)
            proc.wait()
            job.return_code = proc.returncode
            job.status = "completed" if proc.returncode == 0 else "failed"
            _backfill_job_log_from_zlogs(job)
...
def analyze_run(run_name: str) -> dict[str, Any]:
    ...
    out["tags"] = _tags_from_zlogs(run_dir)
    ...
    out["by_area"] = by_area_from_plan_results(out["suite"], agg.get("plan_results") or {})
    return out


def resolve_z_file(run_name: str, filename: str) -> Path | None:
    """Resolve a file under z/<run>/, including FoXYiZ plan subfolders.

    Failure output often cites a bare PNG basename (e.g. PTitle_…_D1_4_….png)
    while the engine stores it under a timestamped plan folder.
    """
    run_dir = (Z_DIR / run_name).resolve()
    if not run_dir.is_dir():
        return None
    rel = str(filename or "").replace("\\", "/").lstrip("/")
    if not rel or ".." in Path(rel).parts:
        return None
    direct = (run_dir / rel).resolve()
    try:
        direct.relative_to(run_dir)
    except ValueError:
        return None
    if direct.is_file():
        return direct
    ...
    if not base or base in {".", ".."}:
        return None
    matches = [p for p in run_dir.rglob(base) if p.is_file()]
    if not matches:
        return None
    if len(matches) == 1:
        return matches[0]
    # Prefer newest when designs re-ran the same plan name
...
def generate_brahl_report_md(
    ...
    if by_area:
        area_table = "| Area | Passed | Failed | Total |\n|------|--------|--------|-------|\n"
        area_table += "\n".join(
            f"| {a.get('area')} | {a.get('passes', 0)} | {a.get('fails', 0)} | {a.get('total', 0)} |"
            for a in by_area
        )
    else:
...
"""
    ...
    thoughts = list(project.get("thought_captures") or [])
    if thoughts:
        thought_lines = "\n".join(
            f"- **{t.get('rating', 'issue').upper()}** — {t.get('title') or 'Untitled'}: "
            f"{(t.get('details') or '')[:160]}"
            for t in thoughts[:40]
        )
    else:
        thought_lines = "*No human-verified thought captures yet.*"
...
"""
    ...
    critical_n = sum(1 for t in thoughts if str(t.get("rating") or "").lower() == "critical")
    issue_n = sum(1 for t in thoughts if str(t.get("rating") or "").lower() == "issue")
    go_auto = fails == 0 and total > 0
    go = go_auto and critical_n == 0
    conclusion = (
        "**GO** — Verify is green and no Critical human findings block launch."
        if go
        else (
            "**NO-GO** — Critical human-verified finding(s) block launch."
            if critical_n
            else "**NO-GO** — Verify has failures; heal yPAD (or document defects) before launch readiness."
        )
    )
...
{exec_summary}
Human-verified: **{critical_n} Critical** · **{issue_n} Issue** · **{len(thoughts)} total** thought capture(s).
...
1. {"Review and heal failing plans listed above." if fails else "No automation failures — review human Critical/Issue captures before publish."}
2. Re-run BRAHL: Step 0 (purpose + context) → Loop 1 → Verify with same fStart config.
3. Expected: Verify green with no Critical human findings; do not weaken assertions to force green.
...
- [{'x' if critical_n == 0 else ' '}] No Critical human-verified findings
- [{'x' if go else ' '}] Conclusion: {'GO' if go else 'NO-GO'}
...
def materialize_brahl_plan_for_suite(
    ...
    *,
    ...
    """Append (default) or rewrite suite y1/y2 from an accepted BRAHL plan."""
    import sys
    ...
    return materialize_brahl_plan_suite(
        suite_name, app_url, brahl_plan, created_by=created_by, merge=merge
    )
