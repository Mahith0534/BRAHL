"""Local project store for BRAHL_Local avatar Build flows."""
...
def _normalize(project: dict[str, Any]) -> dict[str, Any]:
    ...
    project.setdefault("thought_captures", [])
    ...
    if not project.get("suite_name"):
        ...
        for suite in ("BRAHL_Local", "sunshine", "qoa2", "ivvu", "atomic77"):
...
THOUGHT_RATINGS = frozenset({"issue", "critical", "feature"})


def add_thought_capture(
    project_id: str,
    *,
    title: str,
    details: str = "",
    ...
    """Persist a human-verified thought capture (Issue / Critical / Feature)."""
    projects = load_projects()
    for i, p in enumerate(projects):
        if p.get("id") != project_id:
            continue
        p = _normalize(p)
        rate = (rating or "issue").strip().lower()
        if rate not in THOUGHT_RATINGS:
            rate = "issue"
        entry = {
            "id": uuid.uuid4().hex[:10],
        ...
        captures = list(p.get("thought_captures") or [])
        captures.insert(0, entry)
        p["thought_captures"] = captures
        if entry["evidence_ids"]:
            wanted = set(entry["evidence_ids"])
            lib = list(p.get("evidence_library") or [])
            for item in lib:
                if item.get("id") in wanted:
                    item["finding_id"] = entry["id"]
...
def list_thought_captures(
    ...
    *,
    ...
    if not project:
        raise KeyError(project_id)
    items = list(project.get("thought_captures") or [])
    if rating:
        rate = rating.strip().lower()
        items = [t for t in items if str(t.get("rating") or "").lower() == rate]
    return items


def thought_capture_stats(project_id: str) -> dict[str, int]:
    items = list_thought_captures(project_id)
    return {
        "total": len(items),
        "critical": sum(1 for t in items if t.get("rating") == "critical"),
        "issue": sum(1 for t in items if t.get("rating") == "issue"),
        "feature": sum(1 for t in items if t.get("rating") == "feature"),
    }
...
def _resolve_upload_path(project_id: str, report_path: str) -> Path | None:
    ...
    elif norm.startswith("BRAHL_Local/data/uploads/"):
        path = KK_ROOT / norm
    else:
        name = Path(norm).name
    ...
    *,
    ...
    for i, p in enumerate(projects):
        ...
        if write_ypad:
            ...
            ypad_meta: dict[str, Any] = {}
            if suite:
                try:
                    from runner import materialize_brahl_plan_for_suite

                    ypad_meta = materialize_brahl_plan_for_suite(
                        suite,
            ...
            try:
                ...
                if prompt:
                    ...
                    out = updated or get_project(project_id) or p
                    if isinstance(out, dict):
                        out = {**out, "ypad": ypad_meta}
                    return out
            except Exception:
                pass

            out = get_project(project_id) or p
            if isinstance(out, dict):
                out = {**out, "ypad": ypad_meta}
            return out
