...
USAGE_PATH = Path(__file__).resolve().parents[1] / "data" / "ai_usage.json"
...
AI_ROLE_BUDGETS: dict[str, tuple[int, int, int]] = {
    ...
    "planner": (2800, 4, 600),
    "brahl_plan": (1200, 0, 2800),  # structured JSON — no chat history, large completion
    "build": (4200, 6, 800),
...
# Roles that must return full JSON / long structured output (skip ≤120-word chat guardrail)
_STRUCTURED_OUTPUT_ROLES = frozenset({"brahl_plan"})
...
STRUCTURED_GUARDRAIL = (
    "GUARDRAILS: Spell BRAHL/FoXYiZ correctly. Return complete structured output only — "
    "no word-count limit. Never invent locators. No LLM on Run/Loop."
)
...
def is_ai_available() -> bool:
    """True when a chat backend is configured (cloud OpenAI or local OpenAI-compatible)."""
    return bool(os.environ.get("OPENAI_API_KEY", "").strip())


def is_byok_mode() -> bool:
    """True when AI key is local BYOK/desktop (incl. Ollama). Hosted sets QOA_AI_HOSTED=1."""
...
def check_ai_quota(
    ...
    if not is_ai_available():
        return {
            "ok": False,
            "reason": (
                "No AI backend — copy FoXYiZ/f/.env.example to f/.env and set "
                "OPENAI_API_KEY (cloud) or Ollama (OPENAI_BASE_URL + key=ollama)."
            ),
            "code": "no_key",
        }
