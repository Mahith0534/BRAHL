...
def merge_missing(
    ...
    *,
    ...
    """Add rows present in snapshot but missing in current (by key). Never deletes.

    When sheet=plans, also append matching action rows from the snapshot so steps
    arrive with the plans (more tests, merrier).
    """
    dest, manifest = _load_manifest(suite_config, version_id)
    old_rows = _read_snapshot_sheet(dest, sheet, manifest.get("files") or [])
    try:
    ...
    if not added:
        return {
            ...
            "source": source_rel,
            "message": (
                "Nothing to add — every snapshot plan id is already in the live yPAD. "
                "Use Accept plan on Rebuild to append newly generated tests, or Snapshot after edits."
            ),
        }
    ...
    actions_merged = 0
    if sheet == "plans":
        # Bring steps for newly added PlanIds from the same snapshot
        try:
            old_actions = _read_snapshot_sheet(dest, "actions", manifest.get("files") or [])
            act_paths = _resolve_sheet_paths(suite_config, "actions")
            act_target = act_paths[0]
            for p in act_paths:
                if "_journey" not in p.name.lower():
                    act_target = p
                    break
            act_rel = repo_rel(act_target).replace("\\", "/")
            act_cur = read_ypad_sheet(suite_config, "actions", source=act_rel)
            act_rows = [dict(r) for r in act_cur["rows"]]
            act_keys = {_row_key("actions", r) for r in act_rows if _row_key("actions", r)}
            plan_ids = set(added)
            for row in old_actions:
                pid = (row.get("PlanId") or "").strip()
                if pid not in plan_ids:
                    continue
                key = _row_key("actions", row)
                if not key or key in act_keys:
                    continue
                clean = {k: v for k, v in row.items() if not str(k).startswith("_")}
                act_rows.append(clean)
                act_keys.add(key)
                actions_merged += 1
            if actions_merged:
                write_ypad_sheet(
    ...
    return {
        ...
        "keys": added,
        "source": source_rel,
        "actions_merged": actions_merged,
        "write": res,
        "message": f"Added {len(added)} plan(s) and {actions_merged} step row(s) from snapshot.",
    }
